...
class SerialInterface:
    ...
    def connect(self, port: str, baud_rate: int = 115200) -> bool:
        ...
        try:
            ...
            # Open with explicit settings and timeouts (URL-aware: supports loop://, rfc2217://)
            self.serial_connection = serial.serial_for_url(
                url=port,
                baudrate=baud_rate,
            ...
            # Stabilize and reset buffers like terminal apps do
            time.sleep(0.2)
            self.flush_buffers()

            # Mark connected before test write/read so helper methods can run
            self.is_connected = True
            self.port = port
            self.baud_rate = baud_rate

            # Some devices need a small wakeup ping
            try:
            ...
            # Test connection conservatively; do not fail hard if device is silent
            ok = self.test_connection()
            if not ok:
                # If the port is open and no exception occurred, still consider connected
                self.logger.warning("No response to VERSION probe; keeping connection open")
                return True
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self._write_raw("VERSION\r\n")
            time.sleep(0.1)

            # Try to read response (non-blocking due to timeout)
    ...
    def write_command(self, command: str) -> bool:
        ...
        """
        # Allow writes immediately after open; rely on serial port state instead of is_connected
        return self._write_raw(command + "\r\n")
